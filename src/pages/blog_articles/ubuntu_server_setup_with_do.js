...
set up your very own server with DigitalOcean, a cloud-hosting
provider!`;
...
const content = (
  <>
    ...
    <p>
      ...
      that provide services to other client computers over the Internet. Your
      computer can be a server if you tried hard enough (albeit an insecure
      one). There are different protocols servers can use to communicate with
      clients, such as HTTP and FTP. Internally, servers run programs that
      handle these communications with the client to perform their function.
    </p>
    <p>
      Nearly every service on the Internet has at least one server behind it.
      Snapchat? Server. This website? Server. Google? Lots and lots of servers.
      Take a webpage server; a server that serves HTML files to the client so
      they can see a webpage (just like this one). The client and server
      communicate over HTTP, working together to get this HTML file on your
      screen.
    </p>
    ...
    <ul>
      ...
      <li>
        A terminal with <code>ssh</code> and <code>ssh-keygen</code>. Git Bash,
        macOS Terminal, and Unix terminals should work
      </li>
      <li>
        DigitalOcean Droplets cost money, starting at $5/mo. However, they often
    ...
    <p>
      <b>
        If you already have an SSH keypair on your local machine, you can skip
        this step.
      </b>{" "}
      SSH is a tool and network protocol that lets us interact with our server
      using a secure shell terminal. To set this up, we must first generate some
      cryptographic keys using <code>ssh-keygen</code>.
    </p>
    <terminal>$ ssh-keygen -t rsa</terminal>
    <p>
      You can enter where you want the keys to be located, but its nice to keep
      them in their default locations at
    ...
    <p>
      Head into the DigitalOcean dashboard, and go to Settings -> Security. From
      there, you should be able to add the SSH public key that you just
    ...
    <p>
      ...
      server. You can think of this as renting your own server for as low as 5
      bucks a month.
    </p>
    <ol>
      ...
      <li>
        <b>
          Select "SSH Keys" for authentication, and check the SSH key that you
          added to DigitalOcean earlier. This allows you to access your Droplet
          via SSH
        </b>
      </li>
      <li>
    ...
    <p>
      ...
      public IPv4 address. On the Droplets dashboard, you should see something
      like this:
    </p>
    ...
    <p>
      ...
      server. From the same local machine you used to create the SSH keys in the
      first step, run the following in terminal:
    </p>
    ...
    <p>
      ...
      Make sure to secure your server and build cool things! I will be coming
      out with security and development tutorials in the future :)
    </p>
  </>
);
